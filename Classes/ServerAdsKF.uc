...
var config float fDelay;                // delay between a message in seconds
...
private final function CheckDelay() {
    // do not do this while testing
    fDelay = fMax(fDelay, 60);
    SaveConfig();
}
    ...
    if (command ~= "HELP" || command ~= "HLP" || command ~= "HALP") {
        ...
        if (mod ~= "style"){
            ...
            class'UtilInfoLines'.static.SendMessage(
                sender,
                self,
                "^w^Message ^b^delay ^w^changed to: ^y^" $ fDelay
            );
            return;
        } else if (mod ~= "disable") {
